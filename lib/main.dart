...
class _GState extends State<GameScreen> with SingleTickerProviderStateMixin {
  ...
  void _updateAsteroids(double dt){
    for(final a in _asteroids){
      ...
      if((a.x-px).abs()<a.r+kPhoenixSize*0.8&&(a.y-py).abs()<a.r+kPhoenixSize*0.8){
        a.dead=true;
        if(!_player.shieldActive){_res.applyDamageHeat(0.08);_coreTemp+=0.05;hapticMedium();
  ...
  void _spawnEnemy(){
    ...
    _enemies.add(Enemy(x:20+_rng.nextDouble()*(_sw-40),y:-kEnemyR,hp:hp,
  ...
  void _updateEnemies(double dt){
    for(final e in _enemies){
      ...
      switch(e.kind){
        ...
        case EnemyKind.parasite:
          ...
          if((e.x-_player.x).abs()<40&&(e.y-_sh*0.72).abs()<40){_res.energy-=11*dt;
            if(_rng.nextDouble()<0.08)_particles.add(Particle(x:e.x,y:e.y,vx:(_player.x-e.x)*1.5,vy:(_sh*0.72-e.y)*1.5,life:0.4,color:cShield,size:5));}
      ...
      if(e.y>_sh*0.87){e.dead=true;_spawnBurst(e.x,e.y,cFire,10);_audio.playExplosion();
        if(!_player.shieldActive){_res.applyDamageHeat(kCoreHeatPerPass);_coreTemp+=kCoreHeatPerPass;
    ...
    for(final bul in _bullets){
      if(bul.isEnemy){
        ...
        if((bul.x-px).abs()<kPhoenixSize&&(bul.y-py).abs()<kPhoenixSize){
          rem.add(bul);
          if(_player.shieldActive){_player.shieldActive=false;hapticLight();_audio.playShieldHit();
            _spawnBurst(px,py,cShield,8);_addFloat(px,py-20,'ESCUDO!',cShield);}
          else{_res.energy-=bul.fromBoss?11:13;_res.applyDamageHeat(bul.fromBoss?0.045:0.06);
            _coreTemp+=bul.fromBoss?0.032:0.042;hapticMedium();_spawnBurst(px,py,cDanger,8);_addFloat(px,py-20,'-E',cDanger);}}
        final nx=_sw/2,ny=_sh*0.87;
        if(_niFrame<=0&&(bul.x-nx).abs()<kNucleusR&&(bul.y-ny).abs()<kNucleusR){
          rem.add(bul);_niFrame=kNucleusIFrame;_res.applyDamageHeat(bul.fromBoss?kBossBulletDmg:kCoreHeatPerHit);
        ...
        for(final e in _enemies){if(e.dead||hit)continue;
          if((bul.x-e.x).abs()<kEnemyR&&(bul.y-e.y).abs()<kEnemyR){
            rem.add(bul);hit=true;final dmg=_fr.active?bul.damage*1.6:bul.damage;e.hp-=dmg;e.hitFlash=1.0;
            if(e.hp<=0){e.dead=true;_score+=_eScore(e.kind);_spawnBurst(e.x,e.y,_eColor(e.kind),16);_audio.playExplosion();_addFloat(e.x,e.y-10,'+${_eScore(e.kind)}',cGold);}}}
...
class GamePainter extends CustomPainter {
  ...
  void _drawNucleus(Canvas canvas){
    ...
    final lp=Paint()..color=tc.withOpacity(0.2)..strokeWidth=0.7;
    canvas.drawLine(Offset(cx-pulse-10,cy),Offset(cx+pulse+10,cy),lp);canvas.drawLine(Offset(cx,cy-pulse-10),Offset(cx,cy+pulse+10),lp);
  ...
  void _arcBar(Canvas canvas,double cx,double cy,double r,double frac,Color c){
    ...
    canvas.drawArc(Rect.fromCircle(center:Offset(cx,cy),radius:r),st,sw2,false,Paint()..color=Colors.white12..style=PaintingStyle.stroke..strokeWidth=4..strokeCap=StrokeCap.round);
    if(frac>0)canvas.drawArc(Rect.fromCircle(center:Offset(cx,cy),radius:r),st,sw2*frac,false,Paint()..color=c..style=PaintingStyle.stroke..strokeWidth=4..strokeCap=StrokeCap.round);}
  void _bolt(Canvas canvas,Paint p,Offset a,Offset b,Random rng){
    final path=Path()..moveTo(a.dx,a.dy);for(int i=1;i<4;i++){final t=i/4;path.lineTo(a.dx+(b.dx-a.dx)*t+(rng.nextDouble()-0.5)*10,a.dy+(b.dy-a.dy)*t+(rng.nextDouble()-0.5)*10);}path.lineTo(b.dx,b.dy);canvas.drawPath(path,p);}
  ...
  void _drawPhoenix(Canvas canvas){
    ...
    if(touching)canvas.drawPath(Path()..moveTo(px-7,py+kPhoenixSize*0.42)..lineTo(px,py+kPhoenixSize*0.42+30)..lineTo(px+7,py+kPhoenixSize*0.42),
          Paint()..shader=LinearGradient(begin:Alignment.topCenter,end:Alignment.bottomCenter,colors:[const Color(0xFFFFDD00),cFire,cFire.withOpacity(0)]).createShader(Rect.fromLTWH(px-8,py+kPhoenixSize*0.4,16,32)));}
  ...
  void _drawEnemies(Canvas canvas){
    for(final e in enemies){if(e.dead)continue;
      ...
      if(sprites.loaded&&img!=null){drawSprite(canvas,img,Offset(e.x,e.y),kEnemyR*2.4,flash:e.hitFlash);}else{_eFallback(canvas,e,c);}
      if(e.maxHp>20){final bw=kEnemyR*2;final frac=(e.hp/e.maxHp).clamp(0.0,1.0);
  ...
  void _eFallback(Canvas canvas,Enemy e,Color c){
    switch(e.kind){
      case EnemyKind.interceptor:canvas.drawOval(Rect.fromCenter(center:Offset(e.x,e.y),width:kEnemyR*1.8,height:kEnemyR*0.9),Paint()..color=c.withOpacity(0.85));canvas.drawCircle(Offset(e.x,e.y),kEnemyR*0.28,Paint()..color=Colors.white.withOpacity(0.9)..maskFilter=const MaskFilter.blur(BlurStyle.normal,4));
      case EnemyKind.frigate:canvas.drawPath(Path()..moveTo(e.x,e.y-kEnemyR*1.1)..lineTo(e.x+kEnemyR*0.5,e.y)..lineTo(e.x,e.y+kEnemyR*1.1)..lineTo(e.x-kEnemyR*0.5,e.y)..close(),Paint()..color=c.withOpacity(0.85));canvas.drawCircle(Offset(e.x,e.y),kEnemyR*0.22,Paint()..color=Colors.white.withOpacity(0.7)..maskFilter=const MaskFilter.blur(BlurStyle.normal,4));
      case EnemyKind.parasite:final p=Path();for(int i=0;i<6;i++){final a=i*pi/3+e.animT*0.2;if(i==0)p.moveTo(e.x+cos(a)*kEnemyR,e.y+sin(a)*kEnemyR);else p.lineTo(e.x+cos(a)*kEnemyR,e.y+sin(a)*kEnemyR);}p.close();canvas.drawPath(p,Paint()..color=c.withOpacity(0.85));canvas.drawCircle(Offset(e.x,e.y),kEnemyR*0.28,Paint()..color=Colors.white.withOpacity(0.85)..maskFilter=const MaskFilter.blur(BlurStyle.normal,3));
      case EnemyKind.corrupter:final p=Path();for(int i=0;i<8;i++){final a=i*pi/4+e.animT*0.5;final r=i.isEven?kEnemyR:kEnemyR*0.42;if(i==0)p.moveTo(e.x+cos(a)*r,e.y+sin(a)*r);else p.lineTo(e.x+cos(a)*r,e.y+sin(a)*r);}p.close();canvas.drawPath(p,Paint()..color=c);canvas.drawCircle(Offset(e.x,e.y),kEnemyR*0.22,Paint()..color=const Color(0xFFFF0000).withOpacity(0.9)..maskFilter=const MaskFilter.blur(BlurStyle.normal,5));}}
  Color _eColor(EnemyKind k)=>switch(k){EnemyKind.interceptor=>cWarlord,EnemyKind.frigate=>cGreen,EnemyKind.parasite=>cShield,EnemyKind.corrupter=>const Color(0xFFFF6600)};
  ...
  void _drawBoss(Canvas canvas,Boss b){
    ...
    else{final c=b.hitFlash>0?Color.lerp(bossColor,Colors.white,b.hitFlash)!:bossColor;
  ...
  void _drawBullets(Canvas canvas){
    for(final b in bullets){
      if(b.isEnemy){final c=b.fromBoss?bossColor:cWarlord;final r=b.fromBoss?9.0:7.0;
      ...
      else{final lc=freezeActive?cIce:laserColor;
        canvas.drawRRect(RRect.fromRectAndRadius(Rect.fromCenter(center:Offset(b.x,b.y),width:10,height:24),const Radius.circular(5)),Paint()..color=lc.withOpacity(0.22)..maskFilter=const MaskFilter.blur(BlurStyle.normal,7));
        canvas.drawRRect(RRect.fromRectAndRadius(Rect.fromCenter(center:Offset(b.x,b.y),width:3.5,height:22),const Radius.circular(2)),Paint()..shader=LinearGradient(begin:Alignment.topCenter,end:Alignment.bottomCenter,colors:[Colors.white,lc,lc.withOpacity(0.5)]).createShader(Rect.fromCenter(center:Offset(b.x,b.y),width:3.5,height:22)));
  ...
  void _drawParticles(Canvas canvas){
    for(final p in particles){final a=(p.life/p.maxLife).clamp(0.0,1.0);
      if(p.isFrost){final fp=Paint()..color=cFrost.withOpacity(a*0.9)..style=PaintingStyle.stroke..strokeWidth=1.2;
  ...
  void _drawFrost(Canvas canvas){
    ...
    canvas.drawRect(Rect.fromLTWH(0,0,sw,sh),Paint()..shader=RadialGradient(center:Alignment.center,radius:0.7,
        colors:[Colors.transparent,cFrost.withOpacity(ft*0.35),cFrost.withOpacity(ft*0.7)],stops:const[0.4,0.7,1.0]).createShader(Rect.fromLTWH(0,0,sw,sh)));
    final rng=Random(42);final cp=Paint()..color=cFrost.withOpacity(ft*0.55)..strokeWidth=1.0..style=PaintingStyle.stroke;
    void crack(double sx,double sy,int d){if(d<=0)return;for(int i=0;i<3;i++){final a=rng.nextDouble()*pi*2;final l=(20+rng.nextDouble()*40)*ft;final ex=sx+cos(a)*l;final ey=sy+sin(a)*l;canvas.drawLine(Offset(sx,sy),Offset(ex,ey),cp);if(rng.nextDouble()<0.5)crack(ex,ey,d-1);}}
    crack(0,0,4);crack(sw,0,4);crack(0,sh,4);crack(sw,sh,4);
    _txt(canvas,'❄  QUENCH CRÍTICO  ❄',Offset(sw/2,sh*0.38),cFrost.withOpacity(0.6+sin(frostT*pi*8)*0.4),18);}
...
class _RingPainter extends CustomPainter {
  ...
  @override void paint(Canvas canvas,Size size){
    ...
    if(fraction>0)canvas.drawArc(Rect.fromCircle(center:Offset(cx,cy),radius:r),-pi/2,pi*2*fraction,false,Paint()..color=active?color.withOpacity(0.5):color..style=PaintingStyle.stroke..strokeWidth=3..strokeCap=StrokeCap.round);
    ...
    final tp=TextPainter(text:TextSpan(text:icon,style:TextStyle(fontSize:18,color:active?color.withOpacity(0.5):ready?color:Colors.white38)),textDirection:TextDirection.ltr)..layout();
...
class _DecisionOverlay extends StatelessWidget {
  ...
  @override Widget build(BuildContext ctx)=>Container(color:Colors.black.withOpacity(0.9),
    child:SafeArea(child:Column(mainAxisAlignment:MainAxisAlignment.center,children:[
      ...
      _GlowText('Stage $stageNum — Ciclo $cycle',color:Colors.white54,size:13),const SizedBox(height:28),
      ...[for(final opt in options)Padding(padding:const EdgeInsets.symmetric(horizontal:24,vertical:8),
        child:GestureDetector(onTap:()=>onSelect(opt),child:Container(width:double.infinity,padding:const EdgeInsets.all(18),
          decoration:BoxDecoration(border:Border.all(color:opt.color,width:1.5),borderRadius:BorderRadius.circular(12),color:opt.color.withOpacity(0.12),boxShadow:[BoxShadow(color:opt.color.withOpacity(0.25),blurRadius:16)]),
          child:Row(children:[Text(opt.emoji,style:const TextStyle(fontSize:28)),const SizedBox(width:16),
            Expanded(child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[
              Text(opt.title,style:TextStyle(color:opt.color,fontSize:16,fontFamily:'Orbitron',fontWeight:FontWeight.bold)),const SizedBox(height:4),
      ...
      const SizedBox(height:20),const Text('(+20 energía al elegir)',style:TextStyle(color:Colors.white30,fontSize:11))])));
}
    ...
    body:SafeArea(child:Center(child:Column(mainAxisAlignment:MainAxisAlignment.center,children:[
      ...
      AnimatedBuilder(animation:_p,builder:(_,__)=>Transform.scale(scale:1+_p.value*0.05,
        child:GestureDetector(onTap:widget.onStart,child:Container(
          ...
          decoration:BoxDecoration(border:Border.all(color:cFire,width:2),borderRadius:BorderRadius.circular(10),color:cFire.withOpacity(0.15),boxShadow:[BoxShadow(color:cFire.withOpacity(0.4),blurRadius:28)]),
...
class _FleetLegend extends StatelessWidget {
  ...
  @override Widget build(BuildContext ctx){
    ...
    return Column(children:[const Text('FLOTA DEL WARLORD',style:TextStyle(color:Colors.white38,fontSize:9,fontFamily:'Orbitron',letterSpacing:2)),const SizedBox(height:10),
      Row(mainAxisAlignment:MainAxisAlignment.center,children:items.map((e)=>Padding(padding:const EdgeInsets.symmetric(horizontal:10),
...
class GameOverScreen extends StatelessWidget {
  ...
  @override Widget build(BuildContext ctx){final nr=score>=best&&score>0;
    return Scaffold(backgroundColor:cBg,body:SafeArea(child:Center(child:Column(mainAxisAlignment:MainAxisAlignment.center,children:[
...
class _Btn extends StatelessWidget {
  ...
  @override Widget build(BuildContext ctx)=>GestureDetector(onTap:onTap,child:Container(
    ...
    decoration:BoxDecoration(border:Border.all(color:color,width:1.5),borderRadius:BorderRadius.circular(8),color:color.withOpacity(0.12),boxShadow:[BoxShadow(color:color.withOpacity(0.3),blurRadius:14)]),
