...
class AudioManager {
  ...
  int _li = 0;
  final _bg = AudioPlayer(), _boss = AudioPlayer(),
        _expl = AudioPlayer(), _explBoss = AudioPlayer(),
        _quench = AudioPlayer(), _alarm = AudioPlayer();
  ...
  Future<void> switchToBoss() async {
    ...
    try { await _bg.stop(); await _boss.setReleaseMode(ReleaseMode.loop);
      await _boss.play(AssetSource('sounds/musica_jefe.mp3'), volume: 0.7); } catch (_) {}
  }
  Future<void> switchToAmbient() async {
    _bossOn = false;
    try { await _boss.stop(); await _bg.setReleaseMode(ReleaseMode.loop);
      await _bg.play(AssetSource('sounds/musica_ambiente.mp3'), volume: 0.5); } catch (_) {}
  }
  Future<void> playLaser({bool triple = false}) async {
    try {
      final p = _laserPool[_li % _laserPool.length]; _li++;
      await p.stop();
  ...
  Future<void> playQuench() async {
    try { await _quench.play(AssetSource('sounds/explosion_jefe.mp3'), volume: 1.0); } catch (_) {}
  }
  void dispose() {
    for (final p in _laserPool) p.dispose();
    _bg.dispose(); _boss.dispose(); _expl.dispose();
    _explBoss.dispose(); _quench.dispose(); _alarm.dispose();
  }
}
...
class Enemy {
  double x, y, hp, maxHp, vx, vy; EnemyKind kind;
  bool dead=false; double hitFlash=0, actionTimer=0, animT=0;
...
class Boss {
  ...
  double hitFlash=0, animT=0, stateTimer=0, chargeGlow=0, burstTimer=0; int burstCount=0;
...
class AIAdapt {
  ...
  void analyze(PlayerBuild b) {
    if (b.damage>15)             aggression   =(aggression+0.18).clamp(1,3.5);
    if (b.fireRate>1.5)          swarmDensity =(swarmDensity+0.22).clamp(1,3.5);
    if (b.shieldEfficiency>1.3)  counterFire  =(counterFire+0.35).clamp(0.5,2.5);
  }
  double spawnInterval(int c) => (2.0-c*0.12).clamp(0.45,2.0)/aggression;
  int    waveSize(int c)      => (1+(c*0.45+swarmDensity*0.4)).floor().clamp(1,6);
}

// ── CHARGE INDICATOR ─────────────────────────────────────
class ChargeIndicator {
  final String label, icon; final Color color;
...
// ── SPRITE DRAW HELPER (no background) ───────────────────
/// Draws a ui.Image centered at [center] with [size],
/// using BlendMode.srcOver so PNG transparency is respected.
void drawSprite(Canvas canvas, ui.Image img, Offset center, double size,
    {double opacity = 1.0, bool flashWhite = false, double flash = 0}) {
  final src = Rect.fromLTWH(0, 0, img.width.toDouble(), img.height.toDouble());
  final dst = Rect.fromCenter(center: center, width: size, height: size);
  final paint = Paint()
    ..filterQuality = FilterQuality.high
    ..isAntiAlias   = true;
  if (flashWhite && flash > 0) {
    // Draw normal then overlay white with blendMode
    canvas.drawImageRect(img, src, dst, paint);
    canvas.drawImageRect(img, src, dst,
        Paint()
          ..filterQuality = FilterQuality.high
          ..colorFilter = ColorFilter.mode(
              Colors.white.withOpacity(flash * 0.65), BlendMode.srcATop));
  } else {
    canvas.drawImageRect(img, src, dst, paint);
  }
}
...
class _AppRootState extends State<AppRoot> {
  ...
  @override Widget build(BuildContext ctx) => switch (_screen) {
    ...
    AppScreen.playing  => GameScreen(
        key: const ValueKey('g'), onGameOver: _gameOver, audio: _audio, sprites: _sprites),
...
class GameScreen extends StatefulWidget {
  ...
  const GameScreen({super.key, required this.onGameOver, required this.audio, required this.sprites});
...
class _GameScreenState extends State<GameScreen> with SingleTickerProviderStateMixin {
  ...
  int    _score=0; double _coreTemp=0, _nucleusIFrame=0;
  double _shootTimer=0, _spawnTimer=0, _powerUpTimer=22.0;
  ...
  // Longer charge times so they're not trivially available
  final _machineGun = ChargeIndicator(label:'RÁFAGA',  icon:'⚡', color:cGold,
      chargeTime:25.0, activeDuration:8.0);
  final _freeze     = ChargeIndicator(label:'FREEZE',  icon:'❄', color:cIce,
      chargeTime:35.0, activeDuration:6.0);
  ...
  void _update(double dt) {
    if (_quenching) { _quenchT+=dt; _updateParticles(dt);
      if (_quenchT>2.5) widget.onGameOver(_score);
      if (mounted) setState((){}); return; }
    if (_frosting) { _frostT+=dt*0.55; _spawnFrostParticles();
    ...
    if (_player.shieldActive) { _player.shieldTimer-=dt;
    ...
    _powerUpTimer-=dt;
    if (_powerUpTimer<=0) { _powerUpTimer=20+_rng.nextDouble()*12; _spawnPowerUp(); }
    _updatePowerUps(dt);
  ...
  void _updatePowerUps(double dt) {
    for (final p in _powerUps) {
      ...
      if (!p.collected) {
        if ((p.x-_player.x).abs()<40&&(p.y-_sh*0.72).abs()<40) {
  ...
  void _spawnEnemy() {
    ...
    _enemies.add(Enemy(
      ...
      vx:(_rng.nextBool()?1:-1)*(55+_rng.nextDouble()*65),
      vy:70+c*4.5+_rng.nextDouble()*18,kind:k));
  }
  ...
  void _fireBossBurst(Boss b) {
    ...
    const spread=0.31;
    for (final off in [0.0,-spread,spread]) {
      _bullets.add(Bullet(b.x,b.y+kBossR,15,isEnemy:true,fromBoss:true,angle:baseAngle+off));
    }
  }
    ...
    for (final bul in _bullets) {
      if (bul.isEnemy) {
        ...
        if ((bul.x-px).abs()<kPhoenixSize&&(bul.y-py).abs()<kPhoenixSize) {
          rem.add(bul);
          if (_player.shieldActive) {
            _player.shieldActive=false; _spawnBurst(px,py,cShield,8); _addFloat(px,py-20,'ESCUDO!',cShield);
          } else {
            _res.energy-=bul.fromBoss?11:13; _res.applyDamageHeat(bul.fromBoss?0.045:0.06);
  ...
  void _triggerBoss() {
    _enemies.clear();
    _boss=Boss(x:_sw/2,y:_sh*0.16,hp:110+_phase.cycleCount*28.0);
    ...
    return Scaffold(backgroundColor:cBg,
      body:Stack(children:[
        ...
        if (_phase.phase==GamePhase.boss&&_bossAlive)
          Positioned(top:8,left:0,right:0,child:Center(child:_GlowText('⚠ FROZEN WARLORD ⚠',color:cWarlord,size:14))),
        if (isTriple)
    ...
    return GestureDetector(onTap:onTap,
      child:SizedBox(width:48,height:48,
        child:CustomPaint(painter:_RingPainter(
            fraction:indicator.fraction,color:c,ready:ready,active:active,icon:indicator.icon))));
  }
}
...
class GamePainter extends CustomPainter {
  ...
  final int score; final ResourceSystem res; final PhaseEngine phase;
  final SpriteCache sprites;
  ...
  void _drawPhoenix(Canvas canvas){
    ...
    if(sprites.loaded&&sprites.player!=null){
      drawSprite(canvas,sprites.player!,Offset(px,py),kPhoenixSize*2.4);
      return;
    }
    // Fallback
  ...
  void _drawEnemies(Canvas canvas){
    for(final e in enemies){if(e.dead)continue;
      ...
      // Glow ring (always drawn, no background)
      canvas.drawCircle(Offset(e.x,e.y),kEnemyR*1.45,
          Paint()..color=c.withOpacity(0.18)..maskFilter=const MaskFilter.blur(BlurStyle.normal,10));
      final img=sprites.enemyImg(e.kind);
      if(sprites.loaded&&img!=null){
        drawSprite(canvas,img,Offset(e.x,e.y),kEnemyR*2.4,
            flashWhite:true,flash:e.hitFlash);
      } else {
  ...
  void _drawBoss(Canvas canvas,Boss b){
    ...
    if(sprites.loaded&&sprites.boss!=null){
      drawSprite(canvas,sprites.boss!,Offset(b.x,b.y),kBossR*2.4,flashWhite:true,flash:b.hitFlash);
    } else {
      ...
      canvas.drawCircle(Offset(b.x,b.y),kBossR*0.18,Paint()..color=cWarlord..maskFilter=const MaskFilter.blur(BlurStyle.normal,12));
    ...
    if(isCharging) _txt(canvas,'⚠ CARGANDO… ${((1-b.chargeGlow)*kBossChargeTime).toStringAsFixed(1)}s',Offset(b.x,b.y+kBossR+18),cDanger,11);
  }

  void _drawBullets(Canvas canvas){
    for(final b in bullets){
      if(b.isEnemy){
        final c=b.fromBoss?cDanger:cWarlord; final r=b.fromBoss?9.0:7.0;
...
class _MenuScreenState extends State<MenuScreen> with SingleTickerProviderStateMixin {
  ...
  @override Widget build(BuildContext ctx)=>Scaffold(backgroundColor:cBg,
    body:SafeArea(child:Center(child:Column(mainAxisAlignment:MainAxisAlignment.center,children:[
      ...
      _FleetLegend(sprites:widget.sprites),
      const SizedBox(height:40),
...
// ── FLEET LEGEND — uses real sprites ─────────────────────
class _FleetLegend extends StatelessWidget {
  ...
  @override Widget build(BuildContext ctx) {
    final items = [
      (EnemyKind.interceptor, sprites.enemy1, cWarlord, 'Interceptor'),
      (EnemyKind.frigate,     sprites.enemy2, cGreen,   'Frigate'),
      (EnemyKind.parasite,    sprites.enemy3, cShield,  'Parasite'),
      (EnemyKind.corrupter,   sprites.enemy4, const Color(0xFFFF6600), 'Corrupter'),
    ];
        ...
        children:items.map((e)=>Padding(
          ...
          child:Column(children:[
            SizedBox(width:52,height:52,
              child: e.$2 != null
                ? RawImage(image:e.$2,width:52,height:52,filterQuality:FilterQuality.high,fit:BoxFit.contain)
                : Container(width:52,height:52,
                    decoration:BoxDecoration(shape:BoxShape.circle,color:e.$3.withOpacity(0.3),
                        border:Border.all(color:e.$3,width:1.5)),
                    child:Center(child:Text('?',style:TextStyle(color:e.$3,fontSize:20))))),
            const SizedBox(height:6),
            Text(e.$4,style:const TextStyle(color:Colors.white54,fontSize:9,fontFamily:'Orbitron')),
          ]))).toList()),
    ]);
  }
}
