...
// Boss telegraph charge time before firing
const double kBossChargeTime    = 1.1;
// I-frame duration for nucleus after hit
const double kNucleusIFrame     = 0.9;
// Each boss bullet damage
const double kBossBulletDamage  = 0.05; // heat fraction per bullet
// Regular bullet nucleus heat
...
class AudioManager {
  ...
  final AudioPlayer _explosionSfx  = AudioPlayer();
  ...
  Future<void> startAmbient() async {
    ...
    try { await _bgMusic.setReleaseMode(ReleaseMode.loop);
      await _bgMusic.play(AssetSource('sounds/musica_ambiente.mp3'), volume:0.5); } catch (_) {}
  }
  Future<void> switchToBossMusic() async {
    if (_bossPlaying) return; _bossPlaying=true;
    try { await _bgMusic.stop();
  ...
  Future<void> switchToAmbientSilent() async {
    try { await _bossMusic.stop(); await _bgMusic.stop(); } catch (_) {}
  }
  void dispose() {
...
class Player {
  ...
  Player(this.x,{PlayerBuild? build})
      : build=build??PlayerBuild(), energy=build?.maxEnergy??100;
}
...
// Boss with state machine for fair telegraph
class Boss {
...
class PowerUp {
  double x, y, animT;
  PowerUpKind kind; bool collected;
...
// ══════════════════════════════════════════════════════════
//  CHARGE INDICATOR (circular ring)
// ══════════════════════════════════════════════════════════
class ChargeIndicator {
  final String label, icon;
  final Color color;
  double chargeTime; // total seconds to fill
  double current=0;  // 0→chargeTime
  bool ready=false;
  bool active=false; // while effect is running
  double activeTimer=0;
  final double activeDuration;

  ChargeIndicator({required this.label,required this.icon,
      required this.color,required this.chargeTime,required this.activeDuration});
  ...
  void update(double dt){
    if(active){
      ...
      if(!ready){ current=(current+dt).clamp(0,chargeTime);
...
class _AppRootState extends State<AppRoot> {
  ...
  @override Widget build(BuildContext ctx)=>switch(_screen){
    ...
    AppScreen.playing  =>GameScreen(key:const ValueKey('g'),onGameOver:_gameOver,audio:_audio),
...
class _IntroScreenState extends State<IntroScreen> with SingleTickerProviderStateMixin {
  ...
  void _tick(){final f=_scenes[_idx].dialogue;
  ...
  void _next(){
    ...
    if(_idx<_scenes.length-1){setState(()=>_idx++);_startScene();}else widget.onDone();}
        ...
        child:Column(children:[
          Expanded(flex:55,child:Stack(children:[
            ...
            Positioned(top:MediaQuery.of(ctx).padding.top+10,right:12,
              child:Text('${_idx+1}/${_scenes.length}',style:const TextStyle(color:Colors.white38,fontSize:10,fontFamily:'Orbitron'))),
          ])),
...
class SpriteCache {
  ...
  Future<void> load() async {
    try {
      player = await _loadImg('assets/images/player.png');
      boss   = await _loadImg('assets/images/boss.png');
      enemy1 = await _loadImg('assets/images/enemy1.png');
      enemy2 = await _loadImg('assets/images/enemy2.png');
      enemy3 = await _loadImg('assets/images/enemy3.png');
      enemy4 = await _loadImg('assets/images/enemy4.png');
      loaded=true;
...
class _GameScreenState extends State<GameScreen> with SingleTickerProviderStateMixin {
  ...
  int    _score=0;
  double _coreTemp=0;
  double _nucleusIFrame=0; // countdown — nucleus invulnerable when >0
  ...
  void _update(double dt){
    ...
    // Shoot
    final effectiveFireRate=_machineGun.active?_player.build.fireRate*2.8:_player.build.fireRate;
    if(_touching&&_res.energy>0){
      _shootTimer-=dt;
      if(_shootTimer<=0){_shootTimer=0.14/effectiveFireRate;_fireLaser();}
    }
  ...
  void _applyPowerUp(PowerUpKind k){
    switch(k){
      ...
      case PowerUpKind.coreArmor:
        _coreTemp=(_coreTemp-0.28).clamp(0,1);_res.heat=(_res.heat-35).clamp(0,100);
  ...
  String _puLabel(PowerUpKind k)=>switch(k){PowerUpKind.rapidFire=>'⚡ RAPID FIRE',PowerUpKind.tripleShot=>'💥 TRIPLE SHOT',PowerUpKind.shield=>'🛡 ESCUDO',PowerUpKind.coreArmor=>'❄ CORE ARMOR',PowerUpKind.energyBoost=>'💚 ENERGÍA +40'};
        ...
        case EnemyKind.parasite:
          ...
          if((e.x-_player.x).abs()<40&&(e.y-_sh*0.72).abs()<40){
            ...
            if(_rng.nextDouble()<0.08)_particles.add(Particle(
                x:e.x,y:e.y,vx:(_player.x-e.x)*1.5,vy:(_sh*0.72-e.y)*1.5,life:0.4,color:cShield,size:5));}
  ...
  // ── BOSS STATE MACHINE (fair & telegraphed) ───────────────
  void _updateBoss(double dt){
    ...
    switch(b.state){
      ...
      case BossState.charging:
        // Telegraph: boss glows brighter
        b.stateTimer+=dt;b.chargeGlow=(b.stateTimer/Boss.chargeTime).clamp(0,1);
        // Slight vibration while charging
        b.y=_sh*0.16+sin(b.animT*12)*3*b.chargeGlow;
        if(b.stateTimer>=Boss.chargeTime){
          b.state=BossState.firing;b.stateTimer=0;
          b.burstCount=0;b.burstTimer=0;}

      case BossState.firing:
        b.stateTimer+=dt;b.burstTimer+=dt;
        // Fire one burst every 0.45s, max 3 bursts per cycle
        if(b.burstTimer>=0.45&&b.burstCount<3){
          b.burstTimer=0;b.burstCount++;
          _fireBossBurst(b);
        }
        if(b.burstCount>=3){
          b.state=BossState.cooldown;b.stateTimer=0;}

      case BossState.cooldown:
        b.x+=b.vx*dt;b.y=_sh*0.16;
        if(b.x<kBossR||b.x>_sw-kBossR)b.vx*=-1;
        b.stateTimer+=dt;b.chargeGlow=0;
        if(b.stateTimer>=Boss.cooldownTime){
          b.state=BossState.moving;b.stateTimer=0;}
    }
  }

  // Fan spread — NOT aimed at nucleus, spread across field
  void _fireBossBurst(Boss b){
    // Center shot (slightly toward player) + two angled side shots
    // Angle toward player
    final px=_player.x;
    final dx=px-b.x;
    final baseAngle=atan2(dx,_sh*0.5)*0.35; // partial aim, not full

    // Fan: center, left 18°, right 18°
    const spread=0.31; // radians ~18°
    ...
    for(final bul in _bullets){
      if(bul.isEnemy){
        ...
        if(_nucleusIFrame<=0&&
          ...
          _spawnBurst(nx,ny,cDanger,6);
        } else if(_nucleusIFrame>0&&
            (bul.x-nx).abs()<kNucleusR&&(bul.y-ny).abs()<kNucleusR){
          // I-frame active — absorb silently
          rem.add(bul);
          _spawnBurst(bul.x,bul.y,cShield.withOpacity(0.5),4);
        }
      } else {
        // Hit enemies
        bool hit=false;
        for(final e in _enemies){
          ...
          if((bul.x-e.x).abs()<kEnemyR&&(bul.y-e.y).abs()<kEnemyR){
            rem.add(bul);hit=true;e.hp-=bul.damage;e.hitFlash=1.0;
            // Freeze: does double damage
            final dmg=_freeze.active?bul.damage*2:bul.damage;
            e.hp-=(dmg-bul.damage); // already subtracted bul.damage above
            if(e.hp<=0){
  ...
  // Tap charge indicators
  void _onTapIndicator(bool isMachineGun){
    if(isMachineGun&&_machineGun.ready) _machineGun.consume();
    else if(!isMachineGun&&_freeze.ready) _freeze.consume();
  }
        ...
        GestureDetector(
          ...
          child:CustomPaint(
            painter:GamePainter(
              ...
              corePulse:_corePulse,laserColor:laserColor,isTriple:isTriple,
    ...
    return GestureDetector(
      ...
      child:SizedBox(width:48,height:48,
        child:CustomPaint(painter:_RingPainter(
          fraction:indicator.fraction,color:c,ready:ready,active:active,
          icon:indicator.icon)))));
  }
}
...
class _RingPainter extends CustomPainter {
  ...
  @override void paint(Canvas canvas,Size size){
    ...
    // Icon text
    final tp=TextPainter(text:TextSpan(text:icon,style:TextStyle(fontSize:18,color:active?color.withOpacity(0.5):ready?color:Colors.white38)),
...
class GamePainter extends CustomPainter {
  ...
  void _bg(Canvas canvas){
    final danger=coreTemp>0.5?Color.lerp(const Color(0xFF001022),const Color(0xFF200008),(coreTemp-0.5)*2)!:const Color(0xFF000814);
    canvas.drawRect(Rect.fromLTWH(0,0,sw,sh),Paint()..shader=LinearGradient(begin:Alignment.topCenter,end:Alignment.bottomCenter,
        colors:[const Color(0xFF000814),const Color(0xFF001022),danger]).createShader(Rect.fromLTWH(0,0,sw,sh)));
    final rng=Random(77);final sp=Paint()..color=Colors.white.withOpacity(0.5);
  ...
  void _drawNucleus(Canvas canvas){
    ...
    // I-frame flash
    final iFrameAlpha=nucleusIFrame>0?0.7:0.0;
    if(nucleusIFrame>0){
      canvas.drawCircle(Offset(cx,cy),pulse+10,
          Paint()..color=cShield.withOpacity(iFrameAlpha*0.5)..maskFilter=const MaskFilter.blur(BlurStyle.normal,15));
    }
    ...
    for(int i=0;i<8;i++){final a=i*pi/4+corePulse*pi*0.5;
      _bolt(canvas,blp,Offset(cx+cos(a)*pulse*0.5,cy+sin(a)*pulse*0.5),Offset(cx+cos(a)*(pulse+22),cy+sin(a)*(pulse+22)),rng);}
  ...
  void _bolt(Canvas canvas,Paint p,Offset a,Offset b,Random rng){
    ...
    for(int i=1;i<4;i++){final t=i/4;path.lineTo(a.dx+(b.dx-a.dx)*t+(rng.nextDouble()-0.5)*10,a.dy+(b.dy-a.dy)*t+(rng.nextDouble()-0.5)*10);}
  ...
  void _drawPhoenix(Canvas canvas){
    ...
    if(sprites.loaded&&sprites.player!=null){
      _drawSprite(canvas,sprites.player!,Offset(px,py),kPhoenixSize*2.4);
      return;
    }
    // Fallback CustomPainter ship
  ...
  void _drawEnemies(Canvas canvas){
    for(final e in enemies){if(e.dead)continue;
      ...
      canvas.drawCircle(Offset(e.x,e.y),kEnemyR*1.5,Paint()..color=c.withOpacity(0.15)..maskFilter=const MaskFilter.blur(BlurStyle.normal,10));
      ...
      if(sprites.loaded&&img!=null){
        ...
        if(e.hitFlash>0){
          ...
          _drawSprite(canvas,img,Offset(e.x,e.y),kEnemyR*2.2);
  ...
  void _drawEnemyFallback(Canvas canvas,Enemy e,Color c){
    switch(e.kind){
      case EnemyKind.interceptor:
  ...
  void _drawBoss(Canvas canvas,Boss b){
    ...
    // Telegraph glow when charging
    if(isCharging){
      canvas.drawCircle(Offset(b.x,b.y),kBossR*2.2,
          Paint()..color=cDanger.withOpacity(b.chargeGlow*0.35)..maskFilter=const MaskFilter.blur(BlurStyle.normal,30));
      // Warning rings expanding
      for(int i=1;i<=3;i++){
        canvas.drawCircle(Offset(b.x,b.y),kBossR*(1.0+i*0.4*b.chargeGlow),
            Paint()..color=cDanger.withOpacity((1-b.chargeGlow)*0.3/(i*0.8))..style=PaintingStyle.stroke..strokeWidth=1.5);
      }
    }
    ...
    if(sprites.loaded&&sprites.boss!=null){
      if(b.hitFlash>0){
        ...
        _drawSprite(canvas,sprites.boss!,Offset(b.x,b.y),kBossR*2.2);
        canvas.drawCircle(Offset(b.x,b.y),kBossR*1.2,Paint()..color=Colors.white.withOpacity(b.hitFlash*0.6)..blendMode=BlendMode.srcATop);
        canvas.restore();
      } else {
        _drawSprite(canvas,sprites.boss!,Offset(b.x,b.y),kBossR*2.2);
      }
    ...
    canvas.drawRRect(RRect.fromRectAndRadius(Rect.fromLTWH(b.x-bw/2,b.y-kBossR*1.3,bw*frac,6),const Radius.circular(3)),Paint()..color=Color.lerp(cDanger,cWarlord,frac)!);
    _txt(canvas,'WARLORD  ${(frac*100).toInt()}%',Offset(b.x,b.y-kBossR*1.45),cWarlord,10);

    // State indicator
    if(isCharging){
      final label='⚠ CARGANDO… ${((1-b.chargeGlow)*kBossChargeTime).toStringAsFixed(1)}s';
      _txt(canvas,label,Offset(b.x,b.y+kBossR+18),cDanger,11);
    }
  }
...
class _DecisionOverlay extends StatelessWidget {
  ...
  @override Widget build(BuildContext ctx)=>Container(color:Colors.black.withOpacity(0.9),
    child:SafeArea(child:Column(mainAxisAlignment:MainAxisAlignment.center,children:[
      ...
      ...options.map((opt)=>Padding(padding:const EdgeInsets.symmetric(horizontal:24,vertical:8),
...
class _MenuScreenState extends State<MenuScreen> with SingleTickerProviderStateMixin {
  ...
  @override void initState(){super.initState();_ctrl=AnimationController(vsync:this,duration:const Duration(milliseconds:1400))..repeat(reverse:true);_pulse=CurvedAnimation(parent:_ctrl,curve:Curves.easeInOut);}
...
class _FleetLegend extends StatelessWidget {
  @override Widget build(BuildContext ctx){
    final items=[(('◐',cWarlord,'Interceptor')),(('◆',cGreen,'Frigate')),(('⬡',cShield,'Parasite')),(('✦',const Color(0xFFFF6600),'Corrupter'))];
    return Column(children:[
      ...
      Row(mainAxisAlignment:MainAxisAlignment.center,children:items.map((e)=>Padding(
        ...
        child:Column(children:[Text(e.$1,style:TextStyle(color:e.$2,fontSize:20,shadows:[Shadow(color:e.$2,blurRadius:8)])),
          const SizedBox(height:4),Text(e.$3,style:const TextStyle(color:Colors.white54,fontSize:9))]))).toList()),
    ]);
  }
}
