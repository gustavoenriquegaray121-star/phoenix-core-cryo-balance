...
// ══════════════════════════════════════════════════════════
// DIFFICULTY — Rediseñada con experiencias distintas
// ══════════════════════════════════════════════════════════
...
class DifficultyConfig {
  ...
  String get name => switch(level){Difficulty.amateur=>'AMATEUR',Difficulty.veterano=>'VETERANO',Difficulty.comandante=>'COMANDANTE'};
  String get emoji => switch(level){Difficulty.amateur=>'🟢',Difficulty.veterano=>'🟡',Difficulty.comandante=>'🔴'};
  String get tagline => switch(level){Difficulty.amateur=>'Aprende el universo. Siéntete poderoso.',Difficulty.veterano=>'El juego empieza a exigirte. ¿Puedes?',Difficulty.comandante=>'Sin piedad. Todo tiene un costo.'};
  Color get color => switch(level){Difficulty.amateur=>cGreen,Difficulty.veterano=>cGold,Difficulty.comandante=>cRed};
  double get scoreMult => switch(level){Difficulty.amateur=>1.0,Difficulty.veterano=>1.6,Difficulty.comandante=>2.8};

  // ── Enemigos ──
  // Amateur: sensación de poder, proyectiles lentos
  // Veterano: presión real, leading básico
  // Comandante: picos de presión, no constante
  ...
  // ── Heat ──
  // Amateur: casi no importa
  // Veterano: importa, pero recuperable
  // Comandante: domina el gameplay
  ...
  double get overheatDuration => switch(level){Difficulty.amateur=>0.5,Difficulty.veterano=>1.2,Difficulty.comandante=>2.0};
  bool get heatAffectsAccuracy => level==Difficulty.veterano||level==Difficulty.comandante;

  // ── Upgrades ──
  // Amateur: frecuentes y generosos
  // Veterano: normales
  // Comandante: trade-offs obligatorios
  bool get forceTradeoffs => level==Difficulty.comandante;
  bool get tripleGuaranteed => level!=Difficulty.comandante;

  // ── Misil Phoenix ──
  bool get hasMissile => level!=Difficulty.amateur;
  double get missileRechargeTime => switch(level){Difficulty.amateur=>999,Difficulty.veterano=>20.0,Difficulty.comandante=>28.0};
...
enum BossState  { moving, charging, firing, cooldown, vulnerable }
...
class Player {
  double x,energy=100,heat=0;bool shieldActive=false;double shieldTimer=0;PlayerBuild build;
...
class Boss {
  double x,y,hp,maxHp,vx;bool dead=false;double hitFlash=0,animT=0,stateTimer=0,chargeGlow=0,burstTimer=0;int burstCount=0;BossState state=BossState.moving;double repelMeter=0;bool phaseShieldActive=false;double phaseShieldTimer=0,vulnerableTimer=0;bool phaseTriggered=false;
...
// ══════════════════════════════════════════════════════════
// HEAT SYSTEM — Core del gameplay Veterano/Comandante
// ══════════════════════════════════════════════════════════
class HeatSystem {
  double heat=0;
  double overheatTimer=0; // tiempo bloqueado tras overheat
  bool isOverheated=false;
  double _warningPulse=0;
  double _warningDir=1;
  ...
  Color get zoneColor=>switch(zone){
    HeatZone.cool=>cIce,HeatZone.warm=>cGold,HeatZone.hot=>cDanger,HeatZone.overheat=>cRed};
  ...
  void update(double dt,PlayerBuild b,DifficultyConfig diff){
    if(isOverheated){
      overheatTimer-=dt;
      if(overheatTimer<=0){isOverheated=false;overheatTimer=0;heat=60;}// tras overheat empieza en 60
      return;
    }
    // Enfriamiento base
    final coolRate=12.0*b.cooling*diff.heatCoolMult;
    heat=(heat-coolRate*dt).clamp(0,100);
    // Pulso visual advertencia
    if(heat>60){_warningPulse+=_warningDir*dt*4;if(_warningPulse>1)_warningDir=-1;if(_warningPulse<0)_warningDir=1;}
  }

  void applyShot(PlayerBuild b,DifficultyConfig diff){
    if(isOverheated)return;
    final heatAdd=kHeatPerShot*b.heatGenMult*diff.heatGenMult;
    heat=(heat+heatAdd).clamp(0,100);
    if(heat>=100){_triggerOverheat(diff);}
  }

  void _triggerOverheat(DifficultyConfig diff){
    isOverheated=true;overheatTimer=diff.overheatDuration;heat=100;hapticHeavy();
  }

  void cool(double amount){heat=(heat-amount).clamp(0,100);}
  double get fraction=>heat/100;
  double get warningPulse=>_warningPulse;

  // Penalización de precisión en heat alto (Veterano+)
  double accuracyPenalty(DifficultyConfig diff){
    if(!diff.heatAffectsAccuracy)return 0;
    if(heat<60)return 0;
    return ((heat-60)/40)*0.08; // máx 8° de desviación
  }
}

// ══════════════════════════════════════════════════════════
// MISSILE SYSTEM — Veterano y Comandante
// ══════════════════════════════════════════════════════════
class MissileSystem {
  double charge=0;
  bool ready=false;
  ...
  void update(double dt){
    if(ready)return;
    charge=(charge+dt).clamp(0,rechargeTime);
    if(charge>=rechargeTime)ready=true;
  }

  bool consume(){
    if(!ready)return false;
    ready=false;charge=0;return true;
  }
...
class PhaseEngine {
  ...
  void update(double dt,{required VoidCallback onDecision,required VoidCallback onBoss}){timer+=dt;if(phase==GamePhase.combat&&timer>=kCombatDuration){cycleCount++;timer=0;if(cycleCount%3==0){phase=GamePhase.boss;onBoss();}else{phase=GamePhase.decision;onDecision();}}}
...
// ── RESOURCE SYSTEM (energy only — heat is in HeatSystem) ─
class ResourceSystem {
  double energy=100,entropy=0;
  void update(double dt,PlayerBuild b){
    energy=(energy+5*dt).clamp(0,b.maxEnergy);
...
class AIAdapt {
  ...
  void updateSpike(double dt,DifficultyConfig diff){
    if(diff.level!=Difficulty.comandante){_spikeMult=1.0;return;}
    _spikeTimer-=dt;
    if(_spikeTimer<=0){
      // Pico cada 15-25s en Comandante
      _spikeMult=_spikeMult>1.0?1.0:1.8;// toggle
      _spikeTimer=15+Random().nextDouble()*10;
    }
  }
  ...
  int waveSize(int c,int s,DifficultyConfig diff){return (1+(c*0.22+swarmDensity*0.18+(s-1)*0.20)).floor().clamp(1,diff.maxWaveSize);}
}
...
void drawSprite(Canvas canvas,ui.Image img,Offset center,double size,{double flash=0}){final src=Rect.fromLTWH(0,0,img.width.toDouble(),img.height.toDouble());final dst=Rect.fromCenter(center:center,width:size,height:size);canvas.drawImageRect(img,src,dst,Paint()..filterQuality=FilterQuality.high..isAntiAlias=true..blendMode=BlendMode.srcOver);if(flash>0){canvas.drawImageRect(img,src,dst,Paint()..filterQuality=FilterQuality.high..blendMode=BlendMode.srcATop..colorFilter=ColorFilter.mode(Colors.white.withOpacity(flash.clamp(0,1)*0.65),BlendMode.srcATop));}}
...
class AppRoot extends StatefulWidget {
  const AppRoot({super.key});@override State<AppRoot> createState()=>_AppRootState();
}
class _AppRootState extends State<AppRoot> {
  AppScreen _screen=AppScreen.dockIntro;
  int _score=0,_currentStage=1;
  int _bestAmateur=0,_bestVeterano=0,_bestComandante=0;
  int _totalRuns=0;
  ...
  void _startWithDiff(DifficultyConfig diff){
    _diff=diff;_currentStage=1;_carry=null;_score=0;_stagesReached=1;_runStart=DateTime.now();
  ...
  void _onStageClear(int score,PlayerBuild build){
    _score=score;_carry=build;_stagesReached=_currentStage;
  ...
  void _gameOver(int score){
    _audio.switchToAmbient();_stagesReached=_currentStage;_save(score);
    setState((){_score=score;_screen=AppScreen.gameOver;});
  }
  void _save(int s)async{
    final key=switch(_diff.level){Difficulty.amateur=>'best_amateur',Difficulty.veterano=>'best_veterano',Difficulty.comandante=>'best_comandante'};
    final p=await SharedPreferences.getInstance();
    final current=p.getInt(key)??0;if(s>current)await p.setInt(key,s);
    _totalRuns++;await p.setInt('total_runs',_totalRuns);
    _loadBests();
  }
  ...
  @override Widget build(BuildContext ctx)=>switch(_screen){
    ...
    AppScreen.gameOver   =>GameOverScreen(score:_score,best:_currentBest,diff:_diff,stagesReached:_stagesReached,runDuration:_runDuration,onRetry:()=>_startWithDiff(_diff),onChangeDiff:()=>setState(()=>_screen=AppScreen.diffSelect),onMenu:()=>setState(()=>_screen=AppScreen.menu)),
    AppScreen.victory    =>VictoryScreen(score:_score,best:_currentBest,diff:_diff,runDuration:_runDuration,onTryNextDiff:(){
      final next=switch(_diff.level){Difficulty.amateur=>const DifficultyConfig(Difficulty.veterano),Difficulty.veterano=>const DifficultyConfig(Difficulty.comandante),Difficulty.comandante=>null};
      if(next!=null)_startWithDiff(next);else setState(()=>_screen=AppScreen.menu);
    },onMenu:()=>setState(()=>_screen=AppScreen.menu)),
  };
}

// ══════════════════════════════════════════════════════════
// DIFF SELECT — Pantalla de selección de dificultad
// ══════════════════════════════════════════════════════════
class DiffSelectScreen extends StatelessWidget {
  ...
  @override Widget build(BuildContext ctx){
    ...
    final bests=[bestAmateur,bestVeterano,bestComandante];
    final features=[
      ['Proyectiles lentos','PowerUps frecuentes','Triple cañón garantizado','Heat casi sin penalización'],
      ['Leading básico de IA','Heat importa — gestión real','Misil Phoenix (recarga 20s)','Jefe 1: ventana de vulnerabilidad'],
      ['Picos de presión (no constante)','Trade-offs en upgrades','Misil Phoenix (recarga 28s)','Heat domina el gameplay','Triple cañón + costo de heat'],
    ];
    return Scaffold(backgroundColor:cBg,body:SafeArea(child:Column(children:[
      const SizedBox(height:24),
      const _GlowText('SELECCIONA DIFICULTAD',color:cIce,size:16),const SizedBox(height:4),
      ...
      Expanded(child:ListView.builder(padding:const EdgeInsets.symmetric(horizontal:20),itemCount:3,itemBuilder:(ctx,i){
        final d=diffs[i];final best=bests[i];final feats=features[i];
        return Padding(padding:const EdgeInsets.only(bottom:14),child:GestureDetector(onTap:()=>onSelect(d),child:Container(padding:const EdgeInsets.all(18),decoration:BoxDecoration(border:Border.all(color:d.color,width:2),borderRadius:BorderRadius.circular(16),color:d.color.withOpacity(0.08),boxShadow:[BoxShadow(color:d.color.withOpacity(0.2),blurRadius:16)]),child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[
          Row(children:[Text('${d.emoji}  ',style:const TextStyle(fontSize:24)),Expanded(child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[Text(d.name,style:TextStyle(color:d.color,fontSize:16,fontFamily:'Orbitron',fontWeight:FontWeight.bold)),Text(d.tagline,style:const TextInheritedTheme(color:Colors.white54,fontSize:10).style?? const TextStyle(color:Colors.white54,fontSize:10))]))]),
          const SizedBox(height:10),
          Wrap(spacing:6,runSpacing:4,children:feats.map((f)=>Container(padding:const EdgeInsets.symmetric(horizontal:8,vertical:3),decoration:BoxDecoration(color:d.color.withOpacity(0.12),borderRadius:BorderRadius.circular(4),border:Border.all(color:d.color.withOpacity(0.4))),child:Text(f,style:TextStyle(color:d.color,fontSize:9,fontFamily:'Orbitron')))).toList()),
          const SizedBox(height:10),
...
class TextInheritedTheme {
  final Color color;final double fontSize;
  const TextInheritedTheme({required this.color,required this.fontSize});
...
class _SCState extends State<StageClearScreen> with SingleTickerProviderStateMixin {
  ...
  @override Widget build(BuildContext ctx)=>Scaffold(backgroundColor:cBg,body:SafeArea(child:Center(child:ScaleTransition(scale:_a,child:Column(mainAxisAlignment:MainAxisAlignment.center,children:[
    ...
    _GlowText(_stages[widget.stageNum-1].name,color:Colors.white54,size:13),const SizedBox(height:16),
...
// ══════════════════════════════════════════════════════════
// VICTORY — Con botón "intentar en veterano" y stats
// ══════════════════════════════════════════════════════════
class VictoryScreen extends StatelessWidget {
  final int score,best;final DifficultyConfig diff;final Duration runDuration;
  final VoidCallback onTryNextDiff,onMenu;
  const VictoryScreen({super.key,required this.score,required this.best,required this.diff,required this.runDuration,required this.onTryNextDiff,required this.onMenu});
  ...
  @override Widget build(BuildContext ctx)=>Scaffold(backgroundColor:cBg,body:SafeArea(child:Center(child:SingleChildScrollView(child:Column(mainAxisAlignment:MainAxisAlignment.center,children:[
    ...
    Container(margin:const EdgeInsets.symmetric(horizontal:32),padding:const EdgeInsets.all(14),decoration:BoxDecoration(color:Colors.white.withOpacity(0.04),borderRadius:BorderRadius.circular(10),border:Border.all(color:Colors.white12)),child:Column(children:[
      _StatRow('SCORE FINAL','$score',cGold),const SizedBox(height:6),
      if(score>=best&&score>0)_StatRow('NUEVO RÉCORD','🏆',cGold),
      _StatRow('MEJOR ANTERIOR','$best',Colors.white38),const SizedBox(height:6),
      _StatRow('TIEMPO TOTAL',_fmtDuration(runDuration),cIce),
      _StatRow('STAGES COMPLETADOS','4/4',cGreen),
      _StatRow('DIFICULTAD',diff.name,diff.color),
    ])),
    ...
    _Btn(label:'MENÚ PRINCIPAL',color:Colors.white38,onTap:onMenu),
    const SizedBox(height:20),
  ])))));
}
...
// ══════════════════════════════════════════════════════════
// GAME OVER — Con stats y loop de retención
// ══════════════════════════════════════════════════════════
class GameOverScreen extends StatelessWidget {
  final int score,best,stagesReached;final DifficultyConfig diff;final Duration runDuration;
  final VoidCallback onRetry,onChangeDiff,onMenu;
  const GameOverScreen({super.key,required this.score,required this.best,required this.diff,required this.stagesReached,required this.runDuration,required this.onRetry,required this.onChangeDiff,required this.onMenu});
  String _fmtDuration(Duration d)=>'${d.inMinutes}:${(d.inSeconds%60).toString().padLeft(2,'0')}';
  @override Widget build(BuildContext ctx){final nr=score>=best&&score>0;
    return Scaffold(backgroundColor:cBg,body:SafeArea(child:Center(child:SingleChildScrollView(child:Column(mainAxisAlignment:MainAxisAlignment.center,children:[
      ...
      Container(margin:const EdgeInsets.symmetric(horizontal:32),padding:const EdgeInsets.all(14),decoration:BoxDecoration(color:Colors.white.withOpacity(0.04),borderRadius:BorderRadius.circular(10),border:Border.all(color:Colors.white12)),child:Column(children:[
        ...
        _StatRow('TIEMPO','${_fmtDuration(runDuration)}',Colors.white54),
      ])),
      const SizedBox(height:24),
      // Loop retención: reintentar primero
      _Btn(label:'⟳  REINTENTAR',color:diff.color,onTap:onRetry),const SizedBox(height:12),
      _Btn(label:'CAMBIAR DIFICULTAD',color:cIce,onTap:onChangeDiff),const SizedBox(height:12),
...
class _MState extends State<MenuScreen> with SingleTickerProviderStateMixin {
  ...
  @override void dispose(){_c.dispose();super.dispose();}
  @override Widget build(BuildContext ctx)=>Scaffold(backgroundColor:cBg,body:SafeArea(child:Center(child:SingleChildScrollView(child:Column(mainAxisAlignment:MainAxisAlignment.center,children:[
    const SizedBox(height:20),_GlowText('PHOENIX CORE',color:cFire,size:36),const SizedBox(height:4),_GlowText('CRYO BALANCE  V5',color:cIce,size:15),const SizedBox(height:4),
    ...
    Container(margin:const EdgeInsets.symmetric(horizontal:24),padding:const EdgeInsets.all(12),decoration:BoxDecoration(color:Colors.white.withOpacity(0.04),borderRadius:BorderRadius.circular(10),border:Border.all(color:Colors.white12)),child:Column(children:[
      ...
      Row(mainAxisAlignment:MainAxisAlignment.spaceEvenly,children:[_RecordChip(emoji:'🟢',name:'AMATEUR',score:widget.bestAmateur,color:cGreen),_RecordChip(emoji:'🟡',name:'VETERANO',score:widget.bestVeterano,color:cGold),_RecordChip(emoji:'🔴',name:'CMDTE',score:widget.bestComandante,color:cRed)]),
    ])),
    const SizedBox(height:18),_FleetLegend(sprites:widget.sprites),const SizedBox(height:24),
    ...
    const SizedBox(height:12),const Text('4 Stages · 4 Jefes · 3 Dificultades\nCada dificultad es una experiencia distinta',textAlign:TextAlign.center,style:TextStyle(color:Colors.white30,fontSize:10,height:1.5)),
    const SizedBox(height:20),
  ])))));
}
class _RecordChip extends StatelessWidget {
  ...
  @override Widget build(BuildContext ctx)=>Column(children:[Text(emoji,style:const TextStyle(fontSize:18)),const SizedBox(height:2),Text(name,style:TextStyle(color:color,fontSize:8,fontFamily:'Orbitron')),Text(score>0?'$score':'---',style:TextStyle(color:color,fontSize:10,fontFamily:'Orbitron',fontWeight:FontWeight.bold))]);
}
...
class _GState extends State<GameScreen> with SingleTickerProviderStateMixin {
  ...
  // Heat overheat visual
  double _overheatPulse=0;
  // Spike warning Comandante
  bool _spikeWarning=false;double _spikeWarnTimer=0;
  ...
  @override void initState(){
    ...
    _heat=HeatSystem();
    _puTimer=_diff.powerUpInterval;
    _gravTimer=_diff.gravTimerSeconds;
  ...
  void _checkBulletsVsNodes(){
    ...
    for(final bul in _bullets){
      ...
      for(int i=0;i<_portalNodes.length;i++){
        ...
        if(dx*dx+dy*dy<(bul.isMissile?40*40:24*24)){
          rem.add(bul);
          node.hp-=bul.isMissile?40:8;
          _spawnBurst(node.x,node.y,bul.isMissile?Colors.orange:Colors.cyanAccent,bul.isMissile?20:5);
          if(node.hp<=0){node.hp=0;node.broken=true;_onNodeBroken(i);}break;
        }
      }
    }
  ...
  void _fireMissile(){
    ...
    _bullets.add(Bullet(_player.x,py,120,angle:0,isMissile:true,speed:700));
    _spawnBurst(_player.x,py,Colors.orange,8);
    _addFloat(_player.x,py-30,'🚀 MISIL PHOENIX',Colors.orange);
    hapticHeavy();
  }
  ...
  void _update(double dt){
    ...
    if(_bossDying){
      _bossDeathTimer+=dt;_bossShakeIntensity*=0.92;_bossShakeX=(_rng.nextDouble()-0.5)*_bossShakeIntensity;_bossShakeY=(_rng.nextDouble()-0.5)*_bossShakeIntensity;
    ...
    // Heat overheat visual
    if(_heat.isOverheated){_overheatPulse+=dt*8;if(_overheatPulse>1)_overheatPulse=0;}else{_overheatPulse=0;}

    // Spike warning Comandante
    if(_diff.level==Difficulty.comandante&&_ai.currentSpike>1.5&&!_spikeWarning){_spikeWarning=true;_spikeWarnTimer=3.0;_addFloat(_sw/2,_sh*0.4,'⚠ OLEADA MASIVA',cRed);}
    if(_spikeWarning){_spikeWarnTimer-=dt;if(_spikeWarnTimer<=0)_spikeWarning=false;}
    ...
    // Disparo con sistema de heat
    if(_touching&&_res.energy>0&&_heat.canShoot){
      _shootTimer-=dt;
      if(_shootTimer<=0){
        final rate=_mg.active?_player.build.fireRate*1.7:_player.build.fireRate;
        final base=kBaseInterval/rate.clamp(1.0,2.0);
        _shootTimer=base;
        _fireLaser();
      }
    }

    // Mover balas — con velocidad según dificultad para balas enemigas
    for(final b in _bullets){
      if(b.isEnemy){b.y+=360*_diff.bulletSpeedMult*dt;}
      else if(b.isMissile){
        // Misil homming básico
        if(_boss!=null&&_bossAlive){final dx=_boss!.x-b.x;b.x+=dx.sign*b.speed*dt*0.3;b.y-=b.speed*dt;}
        else{b.y-=b.speed*dt;}
      }
      else{b.x+=sin(b.angle)*b.speed*dt;b.y-=cos(b.angle)*b.speed*dt;}
    }
    ...
    if(_phase.phase==GamePhase.combat&&!_bossAlive){_spawnTimer-=dt;if(_spawnTimer<=0){_spawnTimer=_ai.spawnInterval(_phase.cycleCount,_stage.stageNum,_diff);for(int i=0;i<_ai.waveSize(_phase.cycleCount,_stage.stageNum,_diff);i++)_spawnEnemy();}}
  ...
  void _fireLaser(){
    ...
    final heatMult=isTriple&&_diff.level==Difficulty.comandante?1.3:1.0;
    // Simular heatMult multiplicando veces de apply
    _heat.applyShot(_player.build,_diff);
    if(heatMult>1)_heat.applyShot(_player.build,_diff);// extra heat en triple Comandante

    _audio.playLaser(triple:isTriple);
    final py=_sh*0.72-kPhoenixSize;
    // Penalización de precisión por heat (Veterano+)
    final penalty=_heat.accuracyPenalty(_diff);

    if(isTriple){
      _bullets.add(Bullet(_player.x,py,_player.build.damage,angle:penalty*(_rng.nextDouble()-0.5)));
      _bullets.add(Bullet(_player.x,py,_player.build.damage*0.7,angle:-0.13+penalty*(_rng.nextDouble()-0.5)));
      _bullets.add(Bullet(_player.x,py,_player.build.damage*0.7,angle:0.13+penalty*(_rng.nextDouble()-0.5)));
    }else{
      _bullets.add(Bullet(_player.x,py,_player.build.damage,angle:penalty*(_rng.nextDouble()-0.5)));
    }
  ...
  void _splitAsteroid(Asteroid a){
    if(a.level<=0)return;final nextLevel=a.level-1;
    // Veterano+: fragmentos en patrón más predecible pero más ángulos
    final count=_diff.level==Difficulty.amateur?nextLevel==1?3:2:nextLevel==1?4:3;
    for(int i=0;i<count;i++){final ang=(pi*2/count)*i;final spd=nextLevel==1?55+_rng.nextDouble()*35:70+_rng.nextDouble()*40;_spawnAsteroid(nextLevel,ox:a.x,oy:a.y,ovx:cos(ang)*spd,ovy:sin(ang)*spd-10);}
  }

  void _updateAsteroids(double dt){
    for(final a in _asteroids){if(a.dead)continue;a.x+=a.vx*dt;a.y+=a.vy*dt;a.angle+=a.spin*dt;if(a.x<a.r||a.x>_sw-a.r)a.vx*=-1;final px=_player.x,py=_sh*0.72;if((a.x-px).abs()<a.r+kPhoenixSize*0.8&&(a.y-py).abs()<a.r+kPhoenixSize*0.8){a.dead=true;if(!_player.shieldActive){_res.energy-=8;_coreTemp+=0.05;hapticMedium();_spawnBurst(px,py,cFire,10);_addFloat(px,py-20,'ASTEROIDE!',cFire);}else{hapticLight();_audio.playShieldHit();_spawnBurst(px,py,cShield,6);}}final nx=_sw/2,ny=_sh*0.87;if((a.x-nx).abs()<a.r+kNucleusR&&(a.y-ny).abs()<a.r+kNucleusR){a.dead=true;if(_niFrame<=0){_niFrame=kNucleusIFrame;_coreTemp+=kCoreHeatPerHit*1.5;hapticHeavy();_addFloat(nx,ny-20,'NÚCLEO HIT!',cDanger);_spawnBurst(nx,ny,cDanger,8);}}if(!a.dead){for(final e in _enemies){if(e.dead)continue;if((a.x-e.x).abs()<a.r+kEnemyR*0.7&&(a.y-e.y).abs()<a.r+kEnemyR*0.7){a.dead=true;e.hp-=15;e.hitFlash=0.8;_spawnBurst(a.x,a.y,Colors.grey,6);_audio.playExplosion(isAsteroid:true);if(e.hp<=0){e.dead=true;_score+=(_eScore(e.kind)~/2*_diff.scoreMult).toInt();_spawnBurst(e.x,e.y,_eColor(e.kind),10);_audio.playExplosion();}break;}}}if(a.y>_sh+a.r)a.dead=true;}
  ...
  void _updatePowerUps(double dt){for(final p in _powerUps){p.y+=65*dt;p.animT+=dt*3;if(!p.collected&&(p.x-_player.x).abs()<40&&(p.y-_sh*0.72).abs()<40){p.collected=true;_applyPU(p.kind);hapticLight();_spawnBurst(p.x,p.y,_puC(p.kind),18);_addFloat(p.x,p.y-10,_puL(p.kind),_puC(p.kind));}}_powerUps.removeWhere((p)=>p.y>_sh+40||p.collected);}
  ...
  void _spawnEnemy(){
    ...
    final base=(18.0+c*5+(s-1)*6)*_diff.enemyHpMult;
    ...
    for(final e in _enemies){
      ...
      switch(e.kind){
        case EnemyKind.interceptor:e.x+=e.vx*dt;e.y+=e.vy*dt;if(e.x<kEnemyR||e.x>_sw-kEnemyR)e.vx*=-1;
          // Veterano+: leading básico
          if(_diff.level!=Difficulty.amateur&&e.y>_sh*0.3){final leadX=_player.x-e.x;e.x+=leadX.sign*20*_diff.enemySpeedMult*dt;}
        ...
        case EnemyKind.parasite:e.x+=(_player.x-e.x).sign*48*_diff.enemySpeedMult*dt;e.y+=e.vy*dt*0.6;if((e.x-_player.x).abs()<40&&(e.y-_sh*0.72).abs()<40){_res.energy-=11*dt;if(_rng.nextDouble()<0.08)_particles.add(Particle(x:e.x,y:e.y,vx:(_player.x-e.x)*1.5,vy:(_sh*0.72-e.y)*1.5,life:0.4,color:cShield,size:5));}
        case EnemyKind.corrupter:e.x+=e.vx*dt;e.y+=e.vy*dt;if(e.x<kEnemyR||e.x>_sw-kEnemyR)e.vx*=-1;if(e.y>_sh*0.65){_heat.applyShot(_player.build,_diff);_addFloat(e.x,e.y,'+HEAT',cDanger);}// Corrupter aumenta heat directamente
      }
  ...
  void _updateBoss(double dt){
    ...
    // Escudo de fase con ventana de vulnerabilidad clara
    if(b.phaseShieldActive){
      b.phaseShieldTimer-=dt;
      if(b.phaseShieldTimer<=0){
        // Transición a ventana vulnerable
        b.phaseShieldActive=false;
        b.state=BossState.vulnerable;
        b.vulnerableTimer=_diff.bossVulnerableWindow;
        _addFloat(b.x,b.y-kBossR-20,'⚡ VULNERABLE — ¡ATACA!',cGold);
        _spawnBurst(b.x,b.y,Colors.yellow,20);hapticHeavy();
      }
    }
    if(b.state==BossState.vulnerable){
      ...
      if(b.vulnerableTimer<=0){b.state=BossState.moving;b.stateTimer=0;}
      return;
    }

    if(_stage.stageNum==4){b.x+=b.vx*dt;b.y=_sh*0.16+sin(b.animT*0.8)*22;if(b.x<kBossR||b.x>_sw-kBossR)b.vx*=-1;b.burstTimer+=dt;final brokenCount=_portalNodes.isEmpty?0:_portalNodes.where((n)=>n.broken).length;final interval=(1.8-(brokenCount*0.32)).clamp(0.45,1.8);if(b.burstTimer>=interval){b.burstTimer=0;_fireBoss(b);}return;}
    ...
    switch(b.state){
      ...
      case BossState.vulnerable:break;// handled above
    }
  }
  ...
  void _fireBoss(Boss b){
    ...
    final bspeed=360*_diff.bulletSpeedMult;

    if(_stage.stageNum==4){
      final predict=_profile.predictability();double targetX=_player.x;
      // Leading según bias del tracker (Veterano+)
      if(_diff.level!=Difficulty.amateur&&predict>0.5){
        final dom=_profile.dominantMove();
        final lead=_diff.level==Difficulty.comandante?70.0:45.0;
        if(dom==MoveType.right)targetX=(_player.x+lead*_diffMult).clamp(0,_sw);
        else if(dom==MoveType.left)targetX=(_player.x-lead*_diffMult).clamp(0,_sw);
      }
      final predAngle=atan2(targetX-b.x,_sh*0.5)*0.45;
      _bullets.add(Bullet(b.x,b.y+kBossR,7*_diff.bossBulletDmgMult,isEnemy:true,fromBoss:true,angle:predAngle,speed:bspeed));
      final brokenCount=_portalNodes.isEmpty?0:_portalNodes.where((n)=>n.broken).length;
      if(brokenCount>=2||_diff.boss4TripleFromStart){_bullets.add(Bullet(b.x,b.y+kBossR,5*_diff.bossBulletDmgMult,isEnemy:true,fromBoss:true,angle:predAngle-0.22,speed:bspeed));_bullets.add(Bullet(b.x,b.y+kBossR,5*_diff.bossBulletDmgMult,isEnemy:true,fromBoss:true,angle:predAngle+0.22,speed:bspeed));}
    ...
    for(final off in offsets)_bullets.add(Bullet(b.x,b.y+kBossR,dmg,isEnemy:true,fromBoss:true,angle:ba+off,speed:bspeed));
  }
  ...
  void _resolveCollisions(){
    ...
    for(final bul in _bullets){
      if(bul.isEnemy){
        ...
        final nx=_sw/2,ny=_sh*0.87;if(_niFrame<=0&&(bul.x-nx).abs()<kNucleusR&&(bul.y-ny).abs()<kNucleusR){rem.add(bul);_niFrame=kNucleusIFrame;_coreTemp+=bul.fromBoss?0.055:kCoreHeatPerHit;hapticHeavy();_addFloat(nx,ny-20,'NÚCLEO!',cDanger);_spawnBurst(nx,ny,cDanger,6);}
        ...
        bool hit=false;
        for(final e in _enemies){if(e.dead||hit)continue;if((bul.x-e.x).abs()<kEnemyR&&(bul.y-e.y).abs()<kEnemyR){rem.add(bul);hit=true;final dmg=(_fr.active?bul.damage*1.6:bul.damage);e.hp-=dmg;e.hitFlash=1.0;if(e.hp<=0){e.dead=true;_score+=(_eScore(e.kind)*_diff.scoreMult).toInt();_spawnBurst(e.x,e.y,_eColor(e.kind),16);_audio.playExplosion();_addFloat(e.x,e.y-10,'+${(_eScore(e.kind)*_diff.scoreMult).toInt()}',cGold);}}}
        if(!hit&&_bossAlive&&_boss!=null&&!_boss!.dead){
          final bo=_boss!;if((bul.x-bo.x).abs()<kBossR&&(bul.y-bo.y).abs()<kBossR){
            rem.add(bul);
            // Escudo absorbe — vulnerable no
            final inShield=bo.phaseShieldActive;
            final inVulnerable=bo.state==BossState.vulnerable;
            final dmgMult=inShield?0.25:inVulnerable?2.0:1.0;// vulnerable = daño doble
            final actualDmg=(_fr.active?bul.damage*1.5:bul.damage)*dmgMult;
            bo.hp-=actualDmg;bo.hitFlash=1.0;
            _score+=inVulnerable?12:(_fr.active?7:5);
            if(inShield)_spawnBurst(bo.x,bo.y,cShield.withOpacity(0.4),4);
            if(inVulnerable)_spawnBurst(bo.x,bo.y,cGold.withOpacity(0.6),6);
            if(inShield&&bul.isMissile){bo.phaseShieldActive=false;bo.state=BossState.vulnerable;bo.vulnerableTimer=_diff.bossVulnerableWindow;_addFloat(bo.x,bo.y-kBossR-20,'💥 MISIL ROMPE ESCUDO',Colors.orange);}// Misil puede romper el escudo
            if(_stage.stageNum==1&&!bo.phaseTriggered&&bo.hp/bo.maxHp<=0.5){bo.phaseTriggered=true;bo.phaseShieldActive=true;bo.phaseShieldTimer=_diff.bossShieldDuration;_addFloat(bo.x,bo.y-kBossR-20,'⚡ ESCUDO ACTIVO',cShield);_spawnBurst(bo.x,bo.y,cShield,30);hapticHeavy();}
            if(bo.hp<=0&&!_bossDying){
              _bossDying=true;_bossDeathTimer=0;_bossShakeIntensity=18;_timeScale=0.3;
              final bossScore=(500+(_stage.stageNum-1)*200)*_diff.scoreMult;_score+=bossScore.toInt();
              for(int i=0;i<60;i++){final a=_rng.nextDouble()*pi*2;final s=80+_rng.nextDouble()*350;_particles.add(Particle(x:bo.x+(_rng.nextDouble()-0.5)*40,y:bo.y+(_rng.nextDouble()-0.5)*40,vx:cos(a)*s,vy:sin(a)*s,life:0.8+_rng.nextDouble()*1.0,color:_rng.nextBool()?cGold:(_rng.nextBool()?cFire:Colors.white),size:4+_rng.nextDouble()*8));}
              _audio.playExplosion(isBoss:true);hapticHeavy();_addFloat(bo.x,bo.y,'+${bossScore.toInt()} BOSS!',cGold);_addFloat(bo.x,bo.y-50,'★ ELIMINADO ★',_stage.bossColor);_ai.analyze(_player.build,_stage.stageNum);_audio.switchToAmbient();
            }
          }
        }
      }
  ...
  void _triggerBoss(){_enemies.clear();final hp=(_stage.bossHpBase+_phase.cycleCount*_stage.bossHpPerCycle)*_diff.bossHpMult;_boss=Boss(x:_sw/2,y:_sh*0.16,hp:hp);_bossAlive=true;_audio.switchToBoss();if(_stage.hazard==StageHazard.gravityTimer){_gravActive=true;_gravTimer=_diff.gravTimerSeconds;}_addFloat(_sw/2,_sh*0.3,'⚠ ${_stage.bossName}',_stage.bossColor);}
  void _stageClear(){_audio.stopAlarm();_phase.endBoss();if(mounted&&!_disposed)widget.onStageClear(_score,_player.build);}
  void _resetPlayerForStage(){_player.build.hasTripleShot=false;_triplePermanent=false;_player.build.fireRate=_player.build.fireRate.clamp(1.0,1.4);_tripleTimer=0;_boss=null;_bossAlive=false;_bossDying=false;_bossDeathTimer=0;_bullets.clear();_asteroids.clear();_enemies.clear();_portalNodes.clear();_nodesInitialized=false;_tripleGuaranteedUsed=false;_gravTimer=_diff.gravTimerSeconds;_heat.heat=0;_heat.isOverheated=false;}
  ...
  void _selectUpg(UpgradeOption o){
    o.apply(_player.build);_res.energy=(_res.energy+20).clamp(0,_player.build.maxEnergy);_heat.cool(12);
    _upgradeFlash=1.0;_upgradeFlashColor=o.color;_showDec=false;_phase.endDecision();
    if(!_bossAlive&&_phase.cycleCount>=3)_stageClear();
  }

  // ── UPGRADES — Trade-offs en Comandante ──────────────────
  List<UpgradeOption> _buildOpts(){
    ...
    final standard=[
      ...
      UpgradeOption(title:'Cryo Stabilizer',description:'Enfriamiento +80% — heat cae más rápido',emoji:'❄️',color:cIce,apply:(b){b.cooling*=1.8;_heat.cool(35);}),
      UpgradeOption(title:'Void Pulse',description:'Cadencia +25% — más disparos/s',emoji:'⚡',color:cGold,apply:(b)=>b.fireRate=(b.fireRate*1.25).clamp(1,2.0)),
    ...
    // Trade-offs para Comandante — siempre con costo
    final tradeoffs=[
      ...
      UpgradeOption(title:'Rapid Instability',description:'Cadencia +35%',tradeoff:'⚠ Precisión reducida',emoji:'⚡',color:cGold,hasTradeoff:true,apply:(b){b.fireRate=(b.fireRate*1.35).clamp(1,2.0);b.heatGenMult*=1.2;}),
      UpgradeOption(title:'Berserker Mode',description:'+50% daño y cadencia',tradeoff:'⚠ Heat no se enfría mientras disparas',emoji:'🔴',color:cRed,hasTradeoff:true,apply:(b){b.damage*=1.5;b.fireRate=(b.fireRate*1.2).clamp(1,2.0);b.heatGenMult*=1.8;}),
    ...
    if(forceTriple&&!isComandante){
      final t=standard.firstWhere((o)=>o.title=='Triple Cannon');
      final rest=standard.where((o)=>o.title!='Triple Cannon').toList()..shuffle(_rng);
      return [t,...rest.take(2)];
    }
    if(forceTriple&&isComandante){
      final t=tradeoffs.firstWhere((o)=>o.title=='Triple Cannon+');
      final rest=tradeoffs.where((o)=>o.title!='Triple Cannon+').toList()..shuffle(_rng);
      return [t,...rest.take(2)];
    }
  ...
  @override Widget build(BuildContext ctx){
    ...
    return Scaffold(backgroundColor:cBg,body:Stack(children:[
      ...
      // Overheat overlay
      if(_heat.isOverheated)Positioned.fill(child:IgnorePointer(child:AnimatedOpacity(opacity:(_overheatPulse%1)*0.4+0.2,duration:const Duration(milliseconds:50),child:Container(color:cRed.withOpacity(0.15))))),
      if(_heat.isOverheated)Positioned(top:_sh*0.28,left:0,right:0,child:Center(child:Container(padding:const EdgeInsets.symmetric(horizontal:16,vertical:8),decoration:BoxDecoration(color:Colors.black.withOpacity(0.8),borderRadius:BorderRadius.circular(8),border:Border.all(color:cRed,width:2)),child:Column(mainAxisSize:MainAxisSize.min,children:[const Text('🔥 SOBRECALENTAMIENTO 🔥',style:TextStyle(color:cRed,fontSize:13,fontFamily:'Orbitron',fontWeight:FontWeight.bold)),Text('BLOQUEADO ${_heat.overheatTimer.toStringAsFixed(1)}s',style:const TextStyle(color:Colors.white70,fontSize:11,fontFamily:'Orbitron'))])))),
      ...
      if(_boss!=null&&_boss!.state==BossState.vulnerable&&_bossAlive)Positioned(top:70,left:0,right:0,child:Center(child:_GlowText('⚡ VENTANA VULNERABLE — ATACA',color:cGold,size:12))),
...
class _MissilePainter extends CustomPainter {
  ...
  @override void paint(Canvas canvas,Size size){
    ...
    if(ready)canvas.drawCircle(Offset(cx,cy),r*0.65,Paint()..color=Colors.orange.withOpacity(0.25)..maskFilter=const MaskFilter.blur(BlurStyle.normal,6));
...
class GamePainter extends CustomPainter {
  final double sw,sh,coreTemp,corePulse,frostT,quenchT,nucleusIFrame,gravityTimer,portalGrow,mirrorVictoryT,upgradeFlash;
  final bool frosting,quenching,touching,isTriple,freezeActive,machineGunActive,mirrorVictory,glitchActive,paused,bossDying,spikeActive;
  ...
  @override void paint(Canvas canvas,Size size){
    ...
    // Spike warning Comandante
    if(spikeActive)canvas.drawRect(Rect.fromLTWH(0,0,sw,sh),Paint()..color=cRed.withOpacity(0.06));
  ...
  void _bg(Canvas canvas){
    ...
    final rng=Random(77+stageNum);final sp=Paint()..color=Colors.white.withOpacity(0.5);for(int i=0;i<70;i++)canvas.drawCircle(Offset(rng.nextDouble()*sw,rng.nextDouble()*sh),rng.nextDouble()*1.3,sp);
  ...
  void _drawBoss(Canvas canvas,Boss b){
    ...
    // Aura vulnerable — dorada pulsante
    ...
    final img=sprites.bossImg(stageNum);if(sprites.loaded&&img!=null){drawSprite(canvas,img,Offset(b.x,b.y),kBossR*2.4,flash:b.hitFlash);}
    ...
    if(stageNum!=4){const bw=kBossR*2;canvas.drawRRect(RRect.fromRectAndRadius(Rect.fromLTWH(b.x-bw/2,b.y-kBossR*1.3,bw,6),const Radius.circular(3)),Paint()..color=Colors.white24);final frac=(b.hp/b.maxHp).clamp(0.0,1.0);final barC=isVulnerable?cGold:Color.lerp(cDanger,bossColor,frac)!;canvas.drawRRect(RRect.fromRectAndRadius(Rect.fromLTWH(b.x-bw/2,b.y-kBossR*1.3,bw*frac,6),const Radius.circular(3)),Paint()..color=barC);_txt(canvas,isVulnerable?'💥 ${(frac*100).toInt()}%':'${(frac*100).toInt()}%',Offset(b.x,b.y-kBossR*1.45),barC,10);}
  ...
  void _drawHUD(Canvas canvas){
    ...
    // HEAT — barra con 3 zonas de color
    _drawHeatBar(canvas);
  ...
  void _drawHeatBar(Canvas canvas){
    // Barra de heat con 3 colores + zona overheat
    ...
    canvas.drawLine(Offset(x-3,cool40Y),Offset(x+w+3,cool40Y),Paint()..color=cGold.withOpacity(0.5)..strokeWidth=1);
...
class _DecisionOverlay extends StatelessWidget {
  final List<UpgradeOption> options;final int cycle,stageNum;final DifficultyConfig diff;
  ...
  @override Widget build(BuildContext ctx)=>Container(color:Colors.black.withOpacity(0.93),child:SafeArea(child:Column(mainAxisAlignment:MainAxisAlignment.center,children:[
    _GlowText('⚡ FASE DE DECISIÓN',color:cGold,size:19),const SizedBox(height:4),
